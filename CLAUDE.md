...
- **Local LLM via Ollama** at `http://localhost:11434` — model is configurable. The client speaks OpenAI-compatible chat-completions, so the model is swappable by config — Gemini Flash, Groq, vLLM, etc. drop in without code changes. The setup wizard picks a sensible default for the host's hardware.
- **SQLite** (stdlib `sqlite3`) for articles + dedup + feedback. Single-writer process, no concurrency concerns.
- **`feedparser`** for RSS, **`atproto`** for Bluesky, plain `httpx` for HN Algolia.
...
No CI workflows yet — add them when there's a remote and external contributors. Pre-commit hooks similarly.
...
- **The LLM client is OpenAI-compatible by design.** Ollama exposes `/v1/chat/completions`; so do Groq, Gemini-via-OpenRouter, vLLM, etc. Model swap = env var change, no code edit.

## Conventions

- **Source IDs are stable strings** (`hn`, `bsky:handle`, `rss:slug`). Used as foreign keys in storage and filter labels in preferences. Never reuse an ID for a different source.
...
Solo or small-team: push directly to `main`, no PRs.

## Don't add

- **Tests for everything.** Add tests when (a) a bug recurs, (b) a refactor needs a safety net, or (c) an LLM-prompt regression would be expensive. A personal news bot doesn't need 80% coverage.
